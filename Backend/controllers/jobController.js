...
exports.getJobs = async (req, res) => {
  ...
  try {
    ...
    // If userId is provided, add extra info
    if (userId) {
      ...
      return res.json(jobsWithExtra);
    }

    // If no userId, return normal jobs
    res.json(jobs);
  } catch (error) {
...
exports.getJobsEmployer = async (req, res) => {
  try {
    ...
    const jobs = await Job.find({ company: userId })
      ...
      .lean(); //.lean() makes jobs plain JS Object so we can add new field
...
exports.toggleCloseJob = async (req, res) => {
  try {
    ...
    if (job.company.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Not Authorsied to close thi job" });
    }
